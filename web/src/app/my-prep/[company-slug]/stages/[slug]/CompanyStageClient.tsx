...
import { getCompanyBySlug, getCompanyStageBySlug, CompanyConfig } from '../../../../../data/companies';
...
function loadProgress(companySlug: string): StoredProgress | null {
  if (typeof window === 'undefined') return null;
  try {
    const stored = localStorage.getItem(`togtog-progress-${companySlug}`);
...
function saveProgress(companySlug: string, progress: StoredProgress): void {
  if (typeof window === 'undefined') return;
  try {
    localStorage.setItem(`togtog-progress-${companySlug}`, JSON.stringify(progress));
  } catch (e) {
    console.error('Failed to save progress:', e);
  }
}

interface CompanyStageClientProps {
  companySlug: string;
  stageSlug: string;
}

export default function CompanyStageClient({ companySlug, stageSlug }: CompanyStageClientProps) {
  ...
  useEffect(() => {
    ...
    const stageData = getCompanyStageBySlug(companySlug, stageSlug);
    setStage(stageData);

    const allProgress = loadProgress(companySlug);
    if (allProgress && stageData) {
      setStageProgress(allProgress.stages[stageData.id] || null);
    }
    setIsLoaded(true);
  }, [companySlug, stageSlug]);

  // Calculate completion percentage
  const calculateProgress = useCallback((progress: StageProgress | null, stageData: InterviewStage): number => {
    if (!progress || !stageData) return 0;

    const totalSections = Object.keys(SECTION_IDS).length;
    const viewedSections = progress.sectionsViewed.length;

    const totalChecklist = stageData.checklist.length;
  ...
  const updateProgress = useCallback((updater: (prev: StageProgress) => StageProgress) => {
    ...
    setStageProgress((prev) => {
      ...
      const allProgress = loadProgress(companySlug) || { stages: {} };
      allProgress.stages[stage.id] = newProgress;
      saveProgress(companySlug, allProgress);

      return newProgress;
    });
  }, [stage, companySlug, calculateProgress]);
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">The interview stage &quot;{stageSlug}&quot; doesn&apos;t exist.</p>
          <Link
            href={`/my-prep/${companySlug}`}
            className="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700"
          >
            Back to {company?.name || 'Company'} Prep
          </Link>
        </div>
      </div>
    );
  ...
  const sidebarSections = Object.entries(SECTION_IDS).map(([, id]) => ({
    id,
          ...
          <main className="flex-1 space-y-4">
            ...
            <div className="lg:hidden mb-6">
              ...
              <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
                <div
                  ...
                  style={{ width: `${percentComplete}%`, backgroundColor: company.color }}
                />
              </div>
            </div>
                ...
                {stage.hiddenIntent ? (
                  <>
                    ...
                    {stage.assessments && stage.assessments.length > 0 && (
                      <div className="not-prose rounded-lg p-4" style={{ backgroundColor: `${company.color}10`, borderColor: `${company.color}30`, borderWidth: 1 }}>
                        <p className="text-sm font-medium mb-2" style={{ color: company.color }}>They&apos;re assessing:</p>
                        <ul className="space-y-1">
                          {stage.assessments.map((assessment, i) => (
                            <li key={i} className="flex items-center gap-2 text-sm text-gray-800">
                              <svg className="w-4 h-4" style={{ color: company.color }} fill="currentColor" viewBox="0 0 20 20">
                                <path
                                  fillRule="evenodd"
                                  d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                                  clipRule="evenodd"
            ...
            >
              <div className="bg-gray-50 rounded-lg p-6 text-center">
                ...
                <p className="text-sm text-gray-400 mt-1">See what top {company.name} candidates say in this stage</p>
              </div>
            </StageSection>
            ...
            >
              <div className="bg-gray-50 rounded-lg p-6 text-center">
                ...
                <p className="text-gray-500">{company.name}-specific practice questions coming soon...</p>
